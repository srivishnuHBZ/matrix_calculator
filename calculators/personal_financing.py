...
def get_waiver_percentage_pl(settlement_type, repayment_period, ageing_classification, principal_outstanding):
...
def validate_columns(cif_data):
    """Validate that required columns exist for Personal Financing calculations"""
    required_mappings = {
    ...
    if missing:
        st.error(f"Missing required columns for Personal Financing: {', '.join(missing)}")
...
def run_personal_financing_calculator(cif_data):
    """Main Personal Financing calculator logic"""
    ...
    # Principal = O/S balance - Write Off IIS - Memo Late - Late
    ...
    waivers = get_waiver_percentage_pl(settlement_type, repayment_period, ageing_classification, principal_outstanding)
    ...
    repayment_period_months = st.number_input(
        ...
        step=1,
        key="repayment_period_months_input_pl"
    )
    ...
    # Bank Settlement Matrix = 
    # (Principal × (1 - Principal Waiver %)) +
    # (Interest × (1 - Interest Waiver %)) +
    # (Others Charges × (1 - Bank Fee Waiver %)) +
    # (Projected Late Payment × (1 - Late Charges Waiver %)) +
    # Operating Cost
    
    bank_settlement_matrix_amount = (
        (principal_outstanding * (1 - principalWaiverPercent)) +
    ...
    with col_proposed_amount:
        if "proposed_settlement_amount_text_pl" not in st.session_state:
            st.session_state.proposed_settlement_amount_text_pl = "0.00"

        proposed_settlement_amount_str = st.text_input(
            "Proposed Settlement Amount",
            key="proposed_settlement_amount_text_pl"
        )
